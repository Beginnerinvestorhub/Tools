...
export class GamificationService {
  ...
  static async awardPoints(userId: string, points: number, reason: string): Promise<{
    ...
    try {
      ...
      const currentResult = await client.query(
        'SELECT total_points, level FROM user_progress WHERE user_id = $1',
        [userId]
      );
  ...
  // Update user streak
  static async updateStreak(userId: string, streakType: 'login' | 'learning'): Promise<number> {
    const client = await pool.connect();
    ...
    try {
      ...
      const currentResult = await client.query(
        `SELECT ${column}, ${dateColumn} FROM user_progress WHERE user_id = $1`,
        [userId]
      );
      ...
      switch (eventType) {
        ...
        case 'USE_TOOL':
          ...
          const toolsResult = await client.query(
            'SELECT tools_used FROM user_stats WHERE user_id = $1',
            [userId]
          );
          
          const currentTools = toolsResult.rows[0]?.tools_used || [];
  ...
  private static async awardPointsInTransaction(client: PoolClient, userId: string, points: number, reason: string): Promise<void> {
    // Get current progress
    const currentResult = await client.query(
      'SELECT total_points, level FROM user_progress WHERE user_id = $1',
      [userId]
  ...
  // Get leaderboard
  static async getLeaderboard(period: string = 'all_time', limit: number = 10): Promise<any[]> {
    const client = await pool.connect();
